...
public class StringUtils {
	...
	public static String isEmptyCheck(String input) {
		if (input == null || "".equals(input.trim()) || "null".equals(input.trim().toLowerCase()))
			return "";
...
//		if (s==null || !isNumeric(s)) return 0;
		double i = 0;
		try {
	...
	public synchronized static int toInt(String s) {
		int i = 0;
		try {
			if (!isEmpty(s) && s.contains(",")) {
				s = s.replaceAll(",", "");
	...
	public synchronized static String toStrings(double s){
...
//	}
	...
	public static String formatThousandTwo(double number) {
		return formatDouble(number, "###########0.00");
	}
	...
	public static String formatThousandS(String number) {
		...
		if(number.contains(".")){
			...
			return formatDouble(Double.parseDouble(number), "###,###,###,##0.00");
		}
	}
	public static String removerDecimal(String number) {
		if(TextUtils.isEmpty(number)){
			return "0";
		}
	...
	/**
	 *  设置常规数字金额格式
	 * @param number  1234567 或者 1234567.6785
	 * @return 1,234,567  或者 1,234,567.678
	 */
	public static String toFormatNormal(double number){
		NumberFormat format = NumberFormat.getNumberInstance();
		//format.setMinimumFractionDigits(2);//设置数的小数部分所允许的最小位数(如果不足后面补0)
		//format.setMaximumFractionDigits(4);//设置数的小数部分所允许的最大位数(如果超过会四舍五入)
	...
	public static float getNumber2Decimal(double number) {


		try {
			float f = Float.valueOf(formatDouble(number, "0.00"));
			return f;
		}
		catch (Exception e){
			e.printStackTrace();
	...
	public static String convertToWan(double money){


		if (money>=10000){
			return formatThousand(money / 10000) +"万";
		}
		else{
			int ds = (int)money;
			return ds + "元";
	...
	public static String convertToWans(double money){

		if (money>=10000){
			return formatDouble(money / 10000, "###,###,###,##0.00") +"万";
		}
		else{
			return formatDouble(money, "###,###,###,###")+"元";
		}


	}
	public static String convertToWanss(double money){

		if (money>=10000){
			return formatDouble(money / 10000, "###,###,###,##0.00") +"";
		}
		else{
			return formatDouble(money, "###,###,###,###")+"";
		}


	}
	public static String phoneTo344(String phone){
	 ...
	 * @return
	 */
	public static String subStringNumber(String value){
	 ...
	 * @return
	 */
	public static  int  StringsToNumber(String str){
		if(TextUtils.isEmpty(str)){
			return 0;
		}
